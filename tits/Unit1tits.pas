...
эффектом этого явилось пустые буквы. перелопатив этот акал пофиксил это. надолголи хз.
предчуствую завтра снова перевернется. и придется начинать с нначала. поэтому ос
тавляю эту заметку для тебя.
русская Р теперь не отрбражаеца,... и че бля? текскттовик както перекодировался в
другой фармат? но скоро он также скрытно перекодирует себя обратно? как прогуто писать  в
таких условиях????????????????????????????????????????????????
тестить кодировку до считывания с файла??????????????????
 бля, столько времени убил на раскодирование. а после форка в нем нет необходимости.
 это бля источник вечного гемороя
 
                                    как сукаа это может быть????
}
...
{ дальше участок бредаоваго кода. я неполнял как он на форк меняет кодировку текст файла .
 понять этого не возможно.
    ...
    begin
      ...
      for j:=0 to high(c[i]) do
        begin
          ...
          //c[i][j].Caption:=z+utf8to(s[i][2*j])+z;  ///  разьебос на случай если кодированый

          c[i][j].Caption:=s[i][2*j];  ////разьебос на случай если нетронутый
...
begin
  ...
  i:=k div 10000;
  j:=k mod 10000;
  d:=utf8to(s[i][j]);  ///// вот эта сучья строка
  d:=s[i][j];          ///// вот эта сучья строка
